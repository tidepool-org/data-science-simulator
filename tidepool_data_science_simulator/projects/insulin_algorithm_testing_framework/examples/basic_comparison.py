...
def main():
    ...
    # Configure scenarios (smaller range for example)
    config.set('scenarios.initial_bg.range', [100, 110])  # Narrower range for simplicity
    config.set('scenarios.initial_bg.step', 10)
    config.set('scenarios.meal_scenarios.unannounced_meals', [30])  # Two meal sizes
    config.set('scenarios.settings_mismatches.multipliers', [1.05])  # Three settings
    config.set('algorithms.autobolus.partial_application_factors', [0.4])
    config.set('processing.parallel_processes', 14)
